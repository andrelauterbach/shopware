...
import type { uiSidebarOpen } from '@shopware-ag/meteor-admin-sdk/es/ui/sidebar';
import { reactive } from 'vue';

// eslint-disable-next-line sw-deprecation-rules/private-feature-declarations
export type SidebarItemEntry = Omit<uiSidebarOpen, 'responseType'> & {
    baseUrl: string;
    active: boolean;
};
...
const sidebarsStore = Shopware.Store.register({
    ...
    getters: {
        getActiveSidebar(): SidebarItemEntry | null {
            return this.sidebars.find((sidebar) => {
    ...
    actions: {
        ...
        addSidebar({ locationId, title, icon, variant, baseUrl, mode }: SidebarItemEntry) {
